    ...
    describe('ModulesUpdater', () => {
        describe('.update()', () => {
            ...
            it('should unload modules using manager', async() => {
                ...
                await updater.update(['MyModule/foo']);
                assert.deepEqual(FakeManager.lastUnloaded, ['MyModule/foo']);
            });
            ...
            it('should load modules using manager', async() => {
                ...
                await updater.update(['MyModule/bar']);
                assert.deepEqual(FakeManager.lastLoaded, ['MyModule/bar']);
            });
            ...
            it('should remove css modules from themeController', async() => {
                ...
                await updater.update(['css!MyModule/bar']);
                assert.isTrue(spyRemove.calledOnce);
            });

            it('manager should ignore modules that don\'t exist in contents', async() => {
                const manager = new FakeManager();
                const updater = new ModulesUpdater(manager, 'HotReloadUnit/mocks/ThemeController');

                await updater.update(['MyAnotherModule/foo']);
                await updater.update(['css!MyAnotherModule/foo']);
                assert.deepEqual(FakeManager.lastUnloaded, []);
            });
        });
    });
});
